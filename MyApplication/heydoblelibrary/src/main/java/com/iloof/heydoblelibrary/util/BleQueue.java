...
public class BleQueue {
     ...
     *
     * @param queueLen The length of this queue.Fails if queueLen is equal or
     *                 lesser than zero.
     */
     ...
     *
     * @param dataLen The length of data to remove, If the dataLen is greater
     *                than {@link BleQueue#size}, we will
     *                {@link BleQueue#clear()}
     */
     ...
     * @return The element at the specified index, Or 0x00 if the index is
     * out of range.
     */
     ...
     * @return The array at the specified length.But if the arrayLen is
     * greater than {@link BleQueue#size}, we will return all of the
     * data in this queue,if there is no data, we will return null.
     */
    public byte[] getBytes(int arrayLen) {
        ...
        if (arrayLen >= size) {
            ...
            if ((mTop + size) <= length) {
                ...
                System.arraycopy(mQueue, 0, array, remainder, size
                        - remainder);
            }
        } else {
     ...
     *
     * @return We will return all of the data in this queue,if there is no
     * data, we will return null.
     */
    public byte[] getAllBytes() {
        ...
        byte[] array = null;
        if ((null == mQueue) || (0 == size)) {
            // Log.d(TAG, "null == mQueue:" + (null == mQueue) + " size:" +
            // size);
            mRWLock.readLock().unlock(); // UnLock
            return array;
        }
    ...
    /**
     * If you want to know whether is empty about this queue, you can call
     * this funcion.
     *
